...
describe('targeta del dia', () => {
  ...
  test('una solemnitat del temps diu el temps, no la setmana', () => {
    const card = buildDayCard(
      day({Date: new Date(2026, 3, 5), CelebrationType: 'S', LiturgyColor: 'B', GenericLiturgyTime: 'Pasqua', SpecificLiturgyTime: 'Q_DIUM_PASQUA', Week: '1'}),
      {Title: 'Diumenge de Pasqua', Description: ''}, barcelona);
  ...
  test('un dia propi del temps amb títol (Rams) no porta tipus i diu el temps', () => {
    const card = buildDayCard(
      day({Date: new Date(2026, 2, 29), LiturgyColor: 'R', GenericLiturgyTime: 'Quaresma', SpecificLiturgyTime: 'Q_DIUM_RAMS', Week: '6', WeekCycle: '2'}),
      {Title: 'Diumenge de Rams', Description: '-'}, barcelona);
  ...
  test('memòria lliure sense celebrar: en gris, amb l’interruptor apagat', () => {
    const card = buildDayCard(
      ...
      {Title: 'Sants Cosme i Damià, màrtirs', Description: 'Per memòries…'}, barcelona);
  ...
  test('memòria lliure celebrada', () => {
    const card = buildDayCard(
      day({CelebrationType: 'V'}), {Title: 'Memòria de Santa Maria en dissabte', Description: '-'},
  ...
  test('els dies després de Cendra', () => {
    expect(weekText(day({Date: new Date(2026, 1, 18), SpecificLiturgyTime: 'Q_CENDRA', Week: '0'}))).toBe('Dimecres de Cendra');
    expect(weekText(day({Date: new Date(2026, 1, 20), SpecificLiturgyTime: 'Q_CENDRA', Week: '0'}))).toBe('Divendres després de Cendra');
