...
test('l’hora d’ara, amb les mateixes franges d’abans', () => {
  ...
  expect(byHour).toEqual([
    'completes', 'completes', null, null, null, null,
    'laudes', 'laudes', 'laudes',
    'tercia', 'tercia', 'tercia',
    'sexta', 'sexta', 'sexta',
    'nona', 'nona', 'nona',
    'vespres', 'vespres', 'vespres', 'vespres', 'vespres', 'vespres',
  ]);
});
...
test('set fitxes, en l’ordre del dia, i només la d’ara marcada', () => {
  ...
  expect(tiles.map((t) => t.label)).toEqual(['Ofici de lectura', 'Laudes', 'Tèrcia', 'Sexta', 'Nona', 'Vespres', 'Completes']);
  expect(tiles.map((t) => t.screenType)).toEqual(['Ofici', 'Laudes', 'Tèrcia', 'Sexta', 'Nona', 'Vespres', 'Completes']);
