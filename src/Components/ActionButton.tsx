...
export default function ActionButton({label, sublabel, onPress, variant = 'filled', style, accessibilityLabel, testID}: ActionButtonProps) {
        ...
        <Pressable
            ...
            style={({pressed}) => [
                ...
                filled
                    ...
                    : {borderWidth: 1.5, borderColor: colors.accentText},
                pressed ? styles.pressed : null,
                style,
            ]}>
            <Text maxFontSizeMultiplier={theme.maxFontScaleForLabels} style={[styles.label, {color}]}>{label}</Text>
            {sublabel
                ? <Text maxFontSizeMultiplier={theme.maxFontScaleForLabels} style={[styles.sublabel, {color}]}>{sublabel}</Text>
                : null}
        </Pressable>
    );
}
