...
export function buildMass({today, tomorrow, mass, choice}: MassBlockInput): MassBlock {
    ...
    const blessing = today.SpecificLiturgyTime === SpecificLiturgyTimeType.PalmSunday
        ? palmSundayGospel(today.YearType)
        : null;
    if (blessing) {
        return {
            label: 'Missa',
    ...
    return {
        ...
        selector,
        gospel: {
            caption: gospelCaption(vespers && hasContent(mass.Vespers.Title) ? mass.Vespers.Title : null, selected.Gospel.Quote),
