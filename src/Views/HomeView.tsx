...
export default function HomeView(props: {
  ...
  // TODO: thereIsSomeError shouldnt be in the ViewState?
...
function Top_Info(date_getdate, date_getmonth, date_getfullyear) {
  try {
    return (
...
function Info_Liturgica(date_getday) {
  try {

    let text_setmana = "";
    if(CurrentLiturgyDayInformation.Today.Week !== '0' && CurrentLiturgyDayInformation.Today.Week !== '.')
      text_setmana = weekDayName(date_getday) + " de la setmana "
    else if(CurrentLiturgyDayInformation.Today.SpecificLiturgyTime === SpecificLiturgyTimeType.LentAshes){
      if(date_getday === 3)
        text_setmana = weekDayName(date_getday) + " de Cendra"
      else
        text_setmana= weekDayName(date_getday) + " després de Cendra"
    }
    ...
    return (
      <View style={styles.diaLiturgicContainer}>
        <Text style={styles.diaLiturgicText}>
          {text_setmana}
          {
            CurrentLiturgyDayInformation.Today.Week !== '0' && CurrentLiturgyDayInformation.Today.Week !== '.' ?
              liturgicPaint(this.romanize(CurrentLiturgyDayInformation.Today.Week), CurrentLiturgyDayInformation.Today.LiturgyColor)
              :
              null
          }
          {
          ...
          {
            CurrentLiturgyDayInformation.Today.WeekCycle !== '0' && CurrentLiturgyDayInformation.Today.WeekCycle !== '.' ?
              liturgicPaint(CurrentLiturgyDayInformation.Today.YearType, CurrentLiturgyDayInformation.Today.LiturgyColor)
              :
              null
          }
        </Text>
        <Text style={styles.diaLiturgicText}>
          {"Temps - "}
          {liturgicPaint(tempsName(CurrentLiturgyDayInformation.Today.GenericLiturgyTime), CurrentLiturgyDayInformation.Today.LiturgyColor)}
        </Text>
        <Text style={styles.diaLiturgicText}>
          {
            CurrentLiturgyDayInformation.Today.WeekCycle !== '0' && CurrentLiturgyDayInformation.Today.WeekCycle !== '.' ?
          ...
          {
            CurrentLiturgyDayInformation.Today.WeekCycle !== '0' && CurrentLiturgyDayInformation.Today.WeekCycle !== '.' ?
              liturgicPaint(romanize(CurrentLiturgyDayInformation.Today.WeekCycle), CurrentLiturgyDayInformation.Today.LiturgyColor)
              :
              null
          }
        </Text>
      ...
      <View style={styles.cel_container}>
        ...
        {StringManagement.HasLiturgyContent(CurrentCelebrationInformation.Title)?
          <View style={{ paddingBottom: 5, }}>
            {transfromCelTypeName(CurrentLiturgyDayInformation.Today.CelebrationType, CurrentLiturgyDayInformation.Today.GenericLiturgyTime)}
          </View>
          : null
        }
    ...
    return (
      <View style={{ flex: 1, flexDirection: 'row', justifyContent: 'center', paddingTop: 20}}>
        <View style={{ flex: 1, alignItems: 'flex-end', paddingRight: 10, justifyContent: 'flex-start', }}>
          <TouchableOpacity style={styles.two_buttons} onPress={On_Comment_Press}>
            <Icon
              name="ios-mail"
              size={75}
            color={'rgb(50, 50, 50)'} 
        ...
        <View style={{ flex: 1, alignItems: 'flex-start', paddingLeft: 10, justifyContent: 'flex-start', }}>
          <TouchableOpacity style={styles.two_buttons} onPress={On_Give_Press}>
            <Icon
              name="ios-card"
              size={75}
              //color={'#424242'}
...
function liturgicPaint(string, color) {
  try {
    if (color === YearType.B) {
...
function transfromCelTypeName(CT, t) {
  try {
