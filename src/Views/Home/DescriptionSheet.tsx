...
export default function DescriptionSheet({day, visible, onClose}: DescriptionSheetProps) {
    ...
    return (
        <BottomSheet visible={visible} onClose={onClose} accessibilityLabel={day.title} testID="description-sheet">
            ...
            <Text accessibilityRole="header" style={[styles.title, {color: colors.text, fontFamily: theme.fonts.serifSemiBold}]}>
                {day.title}
            </Text>
            <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollContent}>
                <Text selectable={true} style={[styles.description, {color: colors.text}]}>{day.description}</Text>
            </ScrollView>
