...
async function ObtainOfficeCommonPsalter(liturgyDayInformation : LiturgyDayInformation) : Promise<OfficeCommonPsalter> {
    return await SecureCall(async () => {
        if (liturgyDayInformation.Today.SpecificLiturgyTime !== SpecificCelebrationType.Q_TRIDU &&
            liturgyDayInformation.Today.SpecificLiturgyTime !== SpecificCelebrationType.P_OCTAVA &&
            liturgyDayInformation.Today.SpecificLiturgyTime !== SpecificCelebrationType.N_OCTAVA) {
...
async function ObtainInvitationCommonPsalter(liturgyDayInformation : LiturgyDayInformation) : Promise<InvitationCommonPsalter> {
    return await SecureCall(async () => {
        if (liturgyDayInformation.Today.SpecificLiturgyTime === SpecificCelebrationType.O_ORDINARI) {
            const id = (parseInt(liturgyDayInformation.Today.WeekCycle) - 1) * 7 + (liturgyDayInformation.Today.Date.getDay() + 1);
...
async function ObtainCommonPartsUntilFifthWeekOfLentTime(liturgyDayInformation : LiturgyDayInformation) : Promise<CommonPartsUntilFifthWeekOfLentTime> {
    return await SecureCall(async () => {
        if (liturgyDayInformation.Today.SpecificLiturgyTime === SpecificCelebrationType.Q_CENDRA || liturgyDayInformation.Today.SpecificLiturgyTime === SpecificCelebrationType.Q_SETMANES) {
            const id = parseInt(liturgyDayInformation.Today.Week);
...
async function ObtainPartsOfLentTime(liturgyDayInformation : LiturgyDayInformation) : Promise<PartsOfLentTime> {
    return await SecureCall(async () => {
        if (liturgyDayInformation.Today.SpecificLiturgyTime === SpecificCelebrationType.Q_CENDRA) {
...
async function ObtainPartsOfFiveWeeksOfLentTime(liturgyDayInformation : LiturgyDayInformation) : Promise<PartsOfFiveWeeksOfLentTime> {
    return await SecureCall(async () => {
        if (liturgyDayInformation.Today.SpecificLiturgyTime === SpecificCelebrationType.Q_SETMANES) {
            const id = (parseInt(liturgyDayInformation.Today.Week) - 1) * 7 + (liturgyDayInformation.Today.Date.getDay() + 1);
...
async function ObtainCommonPartsOfHolyWeek(liturgyDayInformation : LiturgyDayInformation) : Promise<CommonPartsOfHolyWeek> {
    return await SecureCall(async () => {
        if (liturgyDayInformation.Today.SpecificLiturgyTime === SpecificCelebrationType.Q_DIUM_RAMS || liturgyDayInformation.Tomorrow.SpecificLiturgyTime === SpecificCelebrationType.Q_DIUM_RAMS || liturgyDayInformation.Today.SpecificLiturgyTime === SpecificCelebrationType.Q_SET_SANTA) {
            const id = 1;
...
async function ObtainPalmSundayParts(liturgyDayInformation : LiturgyDayInformation) : Promise<PalmSundayParts> {
    return await SecureCall(async () => {
        if (liturgyDayInformation.Today.SpecificLiturgyTime === SpecificCelebrationType.Q_DIUM_RAMS || liturgyDayInformation.Tomorrow.SpecificLiturgyTime === SpecificCelebrationType.Q_DIUM_RAMS) {
            const id = 1;
...
async function ObtainPartsOfEasterTriduum(liturgyDayInformation : LiturgyDayInformation) : Promise<PartsOfEasterTriduum> {
    return await SecureCall(async () => {
        if (liturgyDayInformation.Today.SpecificLiturgyTime === SpecificCelebrationType.Q_TRIDU || (liturgyDayInformation.Tomorrow.Date.getDay() === 5 && liturgyDayInformation.Tomorrow.SpecificLiturgyTime === SpecificCelebrationType.Q_TRIDU)) {
...
async function ObtainPartsOfEasterBeforeAscension(liturgyDayInformation : LiturgyDayInformation) : Promise<PartsOfEasterBeforeAscension> {
    return await SecureCall(async () => {
        if (liturgyDayInformation.Today.SpecificLiturgyTime === SpecificCelebrationType.P_OCTAVA || liturgyDayInformation.Today.SpecificLiturgyTime === SpecificCelebrationType.P_SETMANES) {
            const id = 1;
            const row = await DatabaseDataService.ObtainMasterRowFromDatabase(PartsOfEasterTriduum.MasterName, id);
...
async function ObtainPartsOfEasterOctave(liturgyDayInformation : LiturgyDayInformation) : Promise<PartsOfEasterOctave> {
    return await SecureCall(async () => {
        if (liturgyDayInformation.Today.SpecificLiturgyTime === SpecificCelebrationType.P_OCTAVA) {
...
async function ObtainPartsOfEasterAfterAscension(liturgyDayInformation : LiturgyDayInformation) : Promise<PartsOfEasterAfterAscension> {
    return await SecureCall(async () => {
        if (liturgyDayInformation.Today.SpecificLiturgyTime === SpecificCelebrationType.P_SETMANES) {
            const id = 1;
...
async function ObtainCommonAdventAndChristmasParts(liturgyDayInformation : LiturgyDayInformation) : Promise<CommonAdventAndChristmasParts> {
    return await SecureCall(async () => {
        if (liturgyDayInformation.Today.SpecificLiturgyTime === SpecificCelebrationType.A_SETMANES ||
            liturgyDayInformation.Today.SpecificLiturgyTime === SpecificCelebrationType.A_FERIES ||
            liturgyDayInformation.Today.SpecificLiturgyTime === SpecificCelebrationType.N_OCTAVA ||
            liturgyDayInformation.Today.SpecificLiturgyTime === SpecificCelebrationType.N_ABANS ||
...
function TodayVespersWillBeFromTomorrowAdventFirstOnes(liturgyDayInformation : LiturgyDayInformation) : boolean{
    return liturgyDayInformation.Tomorrow.Date.getDay() === 0 &&
        ...
        liturgyDayInformation.Tomorrow.SpecificLiturgyTime === SpecificCelebrationType.A_SETMANES;
}

async function ObtainAdventWeekParts(liturgyDayInformation : LiturgyDayInformation) : Promise<AdventWeekParts> {
    return await SecureCall(async () => {
        if (liturgyDayInformation.Today.SpecificLiturgyTime === SpecificCelebrationType.A_SETMANES || TodayVespersWillBeFromTomorrowAdventFirstOnes(liturgyDayInformation)) {
            ...
            let id;
            if (liturgyDayInformation.Today.SpecificLiturgyTime === SpecificCelebrationType.O_ORDINARI && liturgyDayInformation.Tomorrow.SpecificLiturgyTime === SpecificCelebrationType.A_SETMANES) {
                id = 1;
            }
            else {
...
async function ObtainAdventSundayParts(liturgyDayInformation : LiturgyDayInformation) : Promise<AdventSundayParts> {
    return await SecureCall(async () => {
        if (liturgyDayInformation.Today.SpecificLiturgyTime === SpecificCelebrationType.A_SETMANES || liturgyDayInformation.Today.SpecificLiturgyTime === SpecificCelebrationType.A_FERIES) {
...
async function ObtainAdventFairDaysParts(liturgyDayInformation : LiturgyDayInformation) : Promise<AdventFairDaysParts> {
    return await SecureCall(async () => {
        if (liturgyDayInformation.Today.SpecificLiturgyTime === SpecificCelebrationType.A_FERIES) {
...
async function ObtainChristmasWhenOctaveParts(liturgyDayInformation : LiturgyDayInformation) : Promise<ChristmasWhenOctaveParts> {
    return await SecureCall(async () => {
        if (liturgyDayInformation.Today.SpecificLiturgyTime === SpecificCelebrationType.N_OCTAVA && liturgyDayInformation.Today.Date.getDate() !== 25) {
...
async function ObtainChristmasBeforeEpiphanyParts(liturgyDayInformation : LiturgyDayInformation) : Promise<ChristmasBeforeEpiphanyParts> {
    return await SecureCall(async () => {
        if (liturgyDayInformation.Today.SpecificLiturgyTime === SpecificCelebrationType.N_ABANS) {
...
async function ObtainLaudesCommonPsalter(liturgyDayInformation : LiturgyDayInformation) : Promise<LaudesCommonPsalter> {
    return await SecureCall(async () => {
        if (liturgyDayInformation.Today.SpecificLiturgyTime !== SpecificCelebrationType.Q_TRIDU && liturgyDayInformation.Today.SpecificLiturgyTime !== SpecificCelebrationType.P_OCTAVA) {
...
async function ObtainEasterSunday(liturgyDayInformation : LiturgyDayInformation) : Promise<EasterSunday> {
    return await SecureCall(async () => {
        if (liturgyDayInformation.Today.SpecificLiturgyTime === SpecificCelebrationType.P_OCTAVA || liturgyDayInformation.Today.SpecificLiturgyTime === SpecificCelebrationType.Q_DIUM_PASQUA) {
            let id = 1;
...
async function ObtainFiveWeeksOfFirstsVespersOfSundayLentParts(liturgyDayInformation : LiturgyDayInformation) : Promise<FiveWeeksOfSundayLentParts> {
    return await SecureCall(async () => {
        if (liturgyDayInformation.Today.SpecificLiturgyTime === SpecificCelebrationType.Q_SETMANES || liturgyDayInformation.Today.SpecificLiturgyTime === SpecificCelebrationType.Q_CENDRA) {
            let id = 1;
            if (liturgyDayInformation.Today.SpecificLiturgyTime !== SpecificCelebrationType.Q_CENDRA) {
...
async function ObtainSolemnityAndFestivityParts(liturgyDayInformation : LiturgyDayInformation) : Promise<SolemnityAndFestivityParts> {
    return await SecureCall(async () => {
        if (liturgyDayInformation.Today.SpecialCelebration.SolemnityAndFestivityMasterIdentifier !== -1 || liturgyDayInformation.Today.SpecificLiturgyTime === SpecificCelebrationType.Q_TRIDU || liturgyDayInformation.Today.SpecificLiturgyTime === SpecificCelebrationType.N_OCTAVA) {
            let id;
            if (liturgyDayInformation.Today.SpecialCelebration.SolemnityAndFestivityMasterIdentifier === -1 && liturgyDayInformation.Today.SpecificLiturgyTime === SpecificCelebrationType.N_OCTAVA) {
...
async function ObtainCommonNightPrayerPsalter(liturgyDayInformation : LiturgyDayInformation) : Promise<CommonNightPrayerPsalter> {
    return await SecureCall(async () => {
        ...
        if ((liturgyDayInformation.Tomorrow.SpecificLiturgyTime === SpecificCelebrationType.Q_DIUM_PASQUA || liturgyDayInformation.Tomorrow.SpecialCelebration.SolemnityAndFestivityMasterIdentifier !== -1 || liturgyDayInformation.Tomorrow.CelebrationType === CelebrationType.Solemnity) && !(liturgyDayInformation.Today.Date.getDay() === 6 || liturgyDayInformation.Today.Date.getDay() === 0)) {
            id = 8;
        }
        ...
        if (liturgyDayInformation.Today.SpecificLiturgyTime === SpecificCelebrationType.N_OCTAVA) {
            id = 9;
        }
        if (liturgyDayInformation.Today.SpecificLiturgyTime === SpecificCelebrationType.Q_SET_SANTA && (liturgyDayInformation.Today.Date.getDay() === 4 || liturgyDayInformation.Today.Date.getDay() === 5 || liturgyDayInformation.Today.Date.getDay() === 6)) {
            id = 9;
        }
        if (liturgyDayInformation.Today.SpecificLiturgyTime === SpecificCelebrationType.Q_TRIDU) {
            id = 9;
