...
export function ObtainMassLiturgy(): MassLiturgy {
    let massLiturgy = new MassLiturgy();

    //var today_date = GlobalData.date;
    //var today_string = GlobalFunction.calculeDia(today_date, GlobalData.diocesi, GlobalData.diaMogut, GlobalData.diocesiMogut);

    const idSpecialVespers = GetSpecialVespers(today_date, today_string, GlobalData.ABC);

    let part_row_extra_visperas = {
        Vespers: false,
        VetllaPasqua: false,
    }
    ...
    if (idSpecialVespers == '-1') {
        ...
        const hardcoded_dont_show_vispers =
        ...
        if (hardcoded_dont_show_vispers === false &&
            ...
            const tomorrow_date = new Date(today_date.getFullYear(), today_date.getMonth(), today_date.getDate() + 1);
            const tomorrow_string = GlobalFunction.calculeDia(tomorrow_date, GlobalData.diocesi, GlobalData.dataTomorrow.diaMogut, GlobalData.dataTomorrow.diocesiMogut);
...
function GetLiturgia(today_date, today_string, part_row_extra_visperas, celType, tempsespecific, ABC, diaDeLaSetmana, parImpar, setmana, LT, Set_Soul_CB) {
...
function GetSpecialVespers(today_date, today_string, ABC) {
...
// Returns the special day ID from table "LDSantoral" if there was any
function IsSpecialDay(today_date, paroimpar, ABC) {
...
function IsSpecialLliureDay(today_date){
...
function IsSpecialChristmas(dia){
